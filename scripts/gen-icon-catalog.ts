...
async function collect(): Promise<CatalogIcon[]> {
  ...
  for (const loader of CATEGORY_LOADERS) {
    if (loader.category.startsWith('cloud-')) continue;
    const icons = await loader.load();
    for (const m of icons) out.push({ id: m.id, name: m.name, category: m.category });
  }

  // Cloud-provider icons straight from the static manifests.
  for (const { category, file } of CLOUD_MANIFESTS) {
    const entries = JSON.parse(readFileSync(rel(`public/icons/${file}`), 'utf8')) as Array<{
      id: string;
      name: string;
      file: string;
    }>;
...
/** Build the catalog in memory (deduped + sorted). Pure — no disk writes, so
 *  the drift test can compare it against the committed artifact. */
export async function buildCatalog(): Promise<IconCatalog> {
