...
| `RELAY_MAX_CONFIG_BYTES` | `[tenancy.limits].max_config_bytes` (ceiling on a workspace's style-profile registry, `PUT /api/v1/style-profiles` — 507; the registry also counts toward `storage_quota_bytes`, so this is only the structural guard against the sidecar being used as unaccounted storage; `0` = no ceiling; default 2 MiB) |
