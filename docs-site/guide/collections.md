# Collections

Collections are **containers for your documents** — think of them as
"workspaces inside your workspace." As your library grows, collections let you
group related documents together, give each group a name and a colour, and find
your way around at a glance.

<!-- SCREENSHOT: the document browser grouped by collection, showing 2-3 coloured collection headers with document cards underneath -->

## The basics

- A document belongs to **at most one collection** at a time. A document that
  isn't in any collection simply shows up as **Unassigned**.
- Every collection has a **name**, an optional **accent colour**, and an order
  you control — so the groups that matter most can sit up top.
- Collections organise your library; they never change the documents inside
  them. Moving a document between collections (or removing it from one) leaves
  the document itself untouched.

## Creating and managing collections

Open the **document browser** and switch the **Group by** control to
*Collection*. From there you can:

- **Create a collection** — give it a name (and optionally pick a colour).
- **Rename** or **recolour** a collection from its header menu.
- **Reorder** collections so your most-used groups come first.
- **Delete** a collection — the documents inside it aren't deleted, they just
  return to **Unassigned**.

<!-- SCREENSHOT: the collection header menu open, showing Rename / Recolour / Delete -->

### Adding a document to a collection

Use a document card's **Move to collection** menu to drop it into a collection
(or pull it back out to Unassigned). You can move several documents at once when
you have multiple selected.

<!-- SCREENSHOT: a document card's "Move to collection" submenu listing available collections -->

## Local vs. workspace collections

Collections carry a **scope** that matches the documents they hold:

| Scope | Holds | Lives | When you leave a workspace |
|-------|-------|-------|----------------------------|
| **Local** | Your local (personal) documents | On this device only | Kept — local collections are yours |
| **Workspace** | A connected workspace's documents | With the workspace | Dropped from this device |

A document and its collection must share scope — a local document can only join
a local collection, and a workspace document can only join a workspace
collection. This keeps personal organisation separate from a shared workspace's,
and means signing out of a workspace never disturbs your local collections.

## Where collections are headed

Today a collection groups **documents**. The longer-term vision is for a
collection to also be the home for the **assets a group of documents shares** —
think shared file/blob relations and shared style profiles that apply across
every document in the collection. Those are on the roadmap; the grouping you use
today is the foundation they build on.

## See also

- [Multi-Page Documents](/guide/multi-page-documents) — organise *within* a
  single document.
- [Embedded Files](/guide/embedded-files) — the files that live alongside your
  canvas.
- [Collaboration](/guide/collaboration) — sharing a workspace with a team.
