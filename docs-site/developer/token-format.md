---
title: App Token Format
description: The JWT claim shape the relay expects — required claims, the workspace claim, validation order, and JWKS caching.
---
...
| `jti` | Unique token identifier. Used for revocation lookups (see [Token Revocation](./revocation)). |
...
6. Checks `jti` is not in the revocation set (see [Token Revocation](./revocation)).
