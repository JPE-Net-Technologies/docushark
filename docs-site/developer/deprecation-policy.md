...
description: How the relay versions its REST, wire protocol, and MCP surfaces, and how deprecations are announced and sunset.
---

# Deprecation Policy

The relay commits to a published deprecation policy so consumers can plan migrations. This page is part of the wire spec — changes to the policy require the same versioning treatment as the wire itself.
...
- Every deprecation has a migration note on this docs site explaining the recommended replacement path.
...
became a pure OIDC resource server. DocuShark is pre-GA, so this
...
| HS256 token verification path | RS256 + JWKS (see [App Token Format](./token-format)). |
...
- The claim shape documented in [App Token Format](./token-format) — new claims may be added (additive), existing claims may not be removed or repurposed within `v1`.
- The webhook events documented in [Webhooks](./webhooks).
