...
description: Every DocuShark MCP tool — params, returns, concurrency, and limits. The authoritative reference for building against the MCP surface.
---
...
This page is the authoritative reference for the MCP surface. The REST/WS wire
spec lives at [REST API Reference](./rest-api).
...
its discovery doc ride the relay's real origin. `[mcp].port` is ignored.
...
2. **Relay app token (JWT)** — an RS256 token minted by the OIDC issuer;
   ...
   (see [Token Format](./token-format)). This is how a
...
[`scripts/mcp-smoke.sh`](https://github.com/JPE-Net-Technologies/docushark/blob/master/scripts/mcp-smoke.sh).
